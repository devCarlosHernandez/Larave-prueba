    ...
    <div class="card">
      <div class="card-datatable table-responsive pt-0">
        ...
        <div class="d-flex justify-content-end mb-3">
            <a href="{{ route('categorias.create', ['id'=>1]) }}" class="btn btn-primary">
        ...
        <table id="articles" class="table table-striped" style="width:100%">
            ...
            <tbody>
                @forelse ($categorias as $detail)
                    <tr>
                        <td>{{ $detail->id }}</td> <!-- Asegúrate de que esto exista en tu tabla -->
                        <td>{{ $detail->nombre }}</td>
                        <td>{{ $detail->descripcion }}</td>
                        <td>
                            <a href="{{ route('categorias.form', $detail) }}" class="btn btn-warning">
                                <i class="fas fa-pencil-alt me-2"></i> Editar</a>
                        </td>
                        <td>
                            <form action="{{ route('categorias.destroy', $detail) }}" method="post" class="d-inline" id="delete-form-{{ $detail->id }}">
                                @method('DELETE')
                                @csrf
